const getPokemonData = async (id) => {
  ...
  let abilities = data.abilities
    .map(
      (ability) =>
        '<span class="badge rounded-pill text-bg-warning fs-5">' +
  ...
  document.getElementById("lblCharacteristics").innerText =
    characteristic.description;
};

const getCharacteristics = async (pokemon) => {
  let characteristics = await axios.get(
    `https://pokeapi.co/api/v2/characteristic/${pokemon}`
  );
  let characteristic = characteristics.data.descriptions.find(
    (p) => p.language.name == "es"
  );
